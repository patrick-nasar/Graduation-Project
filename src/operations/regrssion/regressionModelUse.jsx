...
export const RegressionModelUse = () => {
    ...
    const handleSubmitCSVFile = (event) => {
        ...
        api.post(`/regression/regression_test_data_obj_or_csv/${classificationId.id}`, { file: file }, {
            headers: {
    ...
    const handleSubmit = (event) => {
        ...
        api.post(`/regression/regression_test_data_obj_or_csv/${classificationId.id}`, columns)
            .then((res) => {
                console.log(res)
                setResult(res.data.result)
            })
    ...
    useEffect(() => {
        api.get(`/regression/regression_get_columns_for_test_data/${classificationId.id}`)
            .then((res) => {
                console.log(res)
        ...
        <div className="relative w-full min-h-[120dvh] px-6 space-x-4 text-white flex justify-center items-center text-center z-90 bg-gradient-to-r from-[#00DBDE] to-[#FC00FF] ">
            ...
            {View &&
                <div className=' w-1/2 h-fit px-4 py-5 bg-white rounded-md hover:shadow-xl space-y-6 shadow-md backdrop-blur-md bg-opacity-10 dark:bg-[#1f2937]  '>
                    <h1 className='font-bold text-xl text-left'>Regression Model:</h1>
                    <table>
                        <tbody>
            ...
            {Result &&
                <div className=' w-[35%] h-fit max-h-[90dvh] overflow-y-auto px-4 py-5 bg-white rounded-md space-y-6 shadow-md backdrop-blur-md bg-opacity-10 dark:bg-[#1f2937]  '>
                    <h1 className='font-bold text-xl text-left'>Regression Model Results:</h1>
                    {Result.map((row) => (
                        <>
                            <table>
                                <tbody>
