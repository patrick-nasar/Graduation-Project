...
import { GeolocationEditInfo } from './geolocationEditInfo'
...
export const GeoloctionMain = ({ medianPoint, latitude, longitude, title }) => {
  ...
  const handleCreateFlow = () => {
    if (flowDiscription == '' || flowName == '') {
      ...
      if (getadmin() === 'true') {
        ...
        api.post('/geolocation/geolocation_add_flow_for_admin', values)
          .then((res) => {
        ...
        api.post('/geolocation/geolocation_add_flow_for_user', values)
          .then((res) => {
            dispatch(adddata(res.data))
            setOpen(false)
            navigate('/Geolocation_Result')
    ...
    <div >
      ...
      {(() => {
        switch (stepnum) {
          ...
          case 1: return <GeolocationEditInfo csv={csv} setTitleColumn={setTitleColumn} setDirectionColumn={setDirectionColumn} />
